...
class GenericAssistant(IAssistant):
    ...
    def __init__(self, intents, intent_methods={}, model_name="assistant_model", error_threshold=0.7):
    ...
    def request(self, message):
        ...
        if not(float(self.get_probability(message)) > self.error_threshold):
    ...
    def get_probability(self, message):
        ints = self._predict_class(message)

        return ints[0]['probability']
